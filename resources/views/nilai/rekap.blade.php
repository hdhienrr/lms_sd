...
@section('content_kelas')
    ...
    <div class="bg-white border border-gray-100 shadow-xl rounded-3xl overflow-hidden mt-6">
        ...
        <div class="bg-gray-50 px-6 py-6 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
            ...
            <table class="min-w-full border-collapse text-sm text-left">
                ...
                <thead>
                    <tr class="bg-blue-600 text-white uppercase tracking-wider text-xs font-semibold">
                        <th rowspan="2" class="px-4 py-3 border-r border-blue-500 text-center w-12 sticky left-0 bg-blue-600 z-10">No</th>
                        <th rowspan="2" class="px-4 py-3 border-r border-blue-500 text-left min-w-[200px] sticky left-12 bg-blue-600 z-10">Nama Siswa</th>
                        
                        {{-- Group Header: Nilai Mapel --}}
                        <th colspan="{{ count($mapels) }}" class="px-4 py-2 border-b border-blue-500 text-center bg-blue-700">
                ...
                <tbody class="bg-white divide-y divide-gray-100 text-gray-700">
                    ...
                    <tr class="hover:bg-blue-50/30 transition-colors group">
                        ...
                        {{-- No --}}
                        <td class="px-4 py-3 text-center font-medium text-gray-500 bg-gray-50 sticky left-0 border-r border-gray-200 group-hover:bg-blue-50">
                            {{ $idx + 1 }}
                        </td>

                        {{-- Nama Siswa --}}
                        <td class="px-4 py-3 font-bold text-gray-800 whitespace-nowrap bg-gray-50 sticky left-12 border-r border-gray-200 group-hover:bg-blue-50">
                            {{ $siswa->nama }}
                        </td>
                        ...
                        @foreach($mapels as $mp)
                            ...
                            @endphp
                            <td class="px-2 py-3 text-center border-r border-gray-100 {{ $isLow ? 'text-red-600 font-bold bg-red-50' : '' }}">
                                {{ $nilai }}
                            </td>
                        @endforeach
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex flex-wrap gap-4 text-xs text-gray-500">
            <div class="flex items-center"><span class="w-3 h-3 bg-red-100 border border-red-200 mr-1 block"></span> Nilai < 70 (Remedial)</div>
            <div class="flex items-center"><span class="w-3 h-3 bg-gray-800 mr-1 block"></span> Kolom Absensi (H=Hadir, S=Sakit, I=Izin, A=Alpha)</div>
        </div>
