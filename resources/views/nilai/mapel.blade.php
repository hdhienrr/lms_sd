...
@section('content_kelas')
    ...
    <div class="text-center mb-10">
        <h2 class="text-3xl font-bold text-blue-600 mb-2">Input Nilai & Asesmen</h2>
    ...
    <div class="flex flex-col md:flex-row justify-center items-center gap-6 mb-12">
        ...
        <a href="{{ route('rapot.show', $id_kelas) }}" 
           class="bg-gray-900 hover:bg-black text-white px-8 py-4 rounded-full shadow-xl transition-all duration-300 transform hover:-translate-y-1 flex items-center font-bold text-lg">
        ...
        @foreach($mapels as $mp)
            ...
            <a href="{{ route('nilai.input', ['id' => $id_kelas, 'mapel' => $mp->nama]) }}" 
               class="group h-full bg-white border border-gray-100 rounded-3xl shadow-sm hover:shadow-xl hover:border-blue-400 transition-all duration-300 transform hover:-translate-y-2 p-6 flex flex-col items-center justify-center text-center relative overflow-hidden">
                ...
                {{-- Dekorasi Garis Atas --}}
                <div class="absolute top-0 left-0 w-full h-2 {{ $isTema ? 'bg-green-500' : 'bg-blue-500' }}"></div>

                {{-- Ikon Bulat --}}
                <div class="mb-4 p-4 rounded-full {{ $isTema ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600' }}">
                    @if($isTema)
